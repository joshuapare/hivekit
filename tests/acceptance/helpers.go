...
func assertSameNodeID(t *testing.T, goNode hive.NodeID, hivexNode bindings.NodeHandle, msgAndArgs ...interface{}) {
	...
	if goAbsolute != uint64(hivexNode) {
		...
		t.Errorf("%s:\n  gohivex HBIN-relative: %d (0x%x)\n  gohivex absolute:      %d (0x%x)\n  hivex absolute:        %d (0x%x)",
			msg, goNode, goNode, goAbsolute, goAbsolute, hivexNode, hivexNode)
	}
}
...
func assertSameValueID(t *testing.T, goVal hive.ValueID, hivexVal bindings.ValueHandle, msgAndArgs ...interface{}) {
	...
	if goAbsolute != uint64(hivexVal) {
		...
		t.Errorf("%s:\n  gohivex HBIN-relative: %d (0x%x)\n  gohivex absolute:      %d (0x%x)\n  hivex absolute:        %d (0x%x)",
			msg, goVal, goVal, goAbsolute, goAbsolute, hivexVal, hivexVal)
	}
}

// assertNodeListsEqual compares lists of nodes from both implementations
func assertNodeListsEqual(t *testing.T, goNodes []hive.NodeID, hivexNodes []bindings.NodeHandle, msgAndArgs ...interface{}) {
	t.Helper()
...
func assertValueListsEqual(t *testing.T, goVals []hive.ValueID, hivexVals []bindings.ValueHandle, msgAndArgs ...interface{}) {
	t.Helper()
...
// assertInt64Equal compares int64 values from both implementations
func assertInt64Equal(t *testing.T, goVal, hivexVal int64, msgAndArgs ...interface{}) {
	t.Helper()

	if goVal != hivexVal {
		msg := "Int64 values don't match"
		if len(msgAndArgs) > 0 {
			msg = fmt.Sprintf(msgAndArgs[0].(string), msgAndArgs[1:]...)
		}
