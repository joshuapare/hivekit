...
//
// On Darwin, MADV_POPULATE_READ is not available, so we use manual read-through
// with SetPanicOnFault protection.
//
...
func PreFaultPages(data []byte) error {
	...
	// Darwin doesn't have MADV_POPULATE_READ, use manual pre-fault only
...
func manualPreFault(data []byte) (retErr error) {
	...
	// Read through all pages (page size is typically 4KB on Darwin)
