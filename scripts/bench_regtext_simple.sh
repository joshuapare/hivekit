...
# Simple benchmark runner that saves results to CSV
# Usage: ./scripts/bench_regtext_simple.sh [label]

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/.." && pwd)"
OUTPUT_FILE="$REPO_ROOT/bench_results.csv"
...
go test -bench='BenchmarkParseReg_(XP|2003|Win8|2012|Generated|StringHeavy|BinaryHeavy|DWORDHeavy|HeavyEscaping|WithDeletions)' \
    -benchmem -run='^$' -benchtime=200ms 2>&1 | grep "^Benchmark" > "$TEMP_FILE"
...
echo ""
echo "✓ Saved $COUNT benchmark results to $OUTPUT_FILE"
echo ""
echo "Recent results for '$LABEL':"
grep ",$LABEL," "$OUTPUT_FILE" | head -10 | column -t -s','
echo ""
echo "To view comparison report: ./scripts/compare_bench.sh"
